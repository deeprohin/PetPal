...
// Constructor
GenericShoppingWindow::GenericShoppingWindow(sf::Font& font, int& userCoins,
...
void GenericShoppingWindow::loadItems() {
  ...
  for (size_t i = 0; i < itemNames.size(); ++i) {
    Item item(itemNames[i], itemPrices[i],
              0);  // Assuming Item has a constructor Item(name, price, stock)

    // Load texture
    item.texture = std::make_shared<sf::Texture>();
    if (!item.texture->loadFromFile(imagePaths[i])) {
      std::cout << "Error: " << itemNames[i] << " image not found at "
    ...
    int startX =
        (1920 - (150 * numCols + horizontalSpacing * (numCols - 1))) / 2;
    int startY = (1080 - (150 * (itemNames.size() / numCols + 1) +
                          verticalSpacing * (numCols - 1))) /
                 2;
    ...
    description.setPosition(xPos,
                            yPos + item.sprite.getGlobalBounds().height + 10);
    description.setString(item.getName() + " - " +
                          std::to_string(item.getPrice()) + " coins");
...
void GenericShoppingWindow::handleEvents(sf::RenderWindow& window) {
  ...
  while (window.pollEvent(event)) {
    ...
    if (event.type == sf::Event::MouseButtonPressed) {
      ...
      for (auto& item : items) {
        if (item.sprite.getGlobalBounds().contains(static_cast<float>(x),
                                                   static_cast<float>(y))) {
          if (userCoins >= item.getPrice()) {
            userCoins -= item.getPrice();
            basket.addItem(std::make_shared<Item>(item));  // Add item to basket
            coinsText.setString("Coins: " + std::to_string(userCoins));
            trolleyText.setString("Trolley: " +
                                  std::to_string(basket.getSize()));
          } else {
