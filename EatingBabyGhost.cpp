...
void EatingBabyGhost::loadFoodItems() {
  ...
  std::vector<int> itemPrices = {
      50, 30, 70, 500};  // Update prices to include all four items
  ...
  const float startX = (1920 - (columns * horizontalSpacing)) /
  ...
  for (size_t i = 0; i < itemNames.size(); ++i) {
    ...
    // Calculate the scaling factor based on the larger dimension (width or
    // height)
    ...
    item.quantityText.setPosition(
        xPosition, yPosition + item.sprite.getGlobalBounds().height + 15);
