...
EatingAdultGhostWindow::EatingAdultGhostWindow(sf::Font& font, ItemList* basket,
  ...
  window =
      new sf::RenderWindow(sf::VideoMode(1920, 1080), "Eating Ghost Window");
...
void EatingAdultGhostWindow::loadFoodItems() {
  ...
  const float startX =
      (1920 - (columns * imageSize + (columns - 1) * horizontalSpacing)) /
  ...
  for (size_t i = 0; i < itemNames.size(); ++i) {
    ...
    item.sprite.setScale(
        imageSize / item.sprite.getLocalBounds().width,
        imageSize /
            item.sprite.getLocalBounds().height);  // Scale to fixed size
    ...
    item.quantityText.setPosition(
        ...
        yPosition + item.sprite.getGlobalBounds().height +
            5  // Increased spacing below the image
    );
...
// Play an animation or sound for consuming food (not implemented in this
// example)
