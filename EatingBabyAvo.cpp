...
// Constructor
EatingBabyAvo::EatingBabyAvo(sf::Font& font, ItemList* basket, int& basketSize,
  ...
  backgroundSprite.setScale(
      static_cast<float>(1920) /
          backgroundTexture.getSize().x,  // Scale to fit width
      static_cast<float>(1080) /
          backgroundTexture.getSize().y  // Scale to fit height
  );
...
void EatingBabyAvo::loadFoodItems() {
  std::vector<std::string> itemNames = {"Baby Pumpkin", "Baby Banana", "Yakult",
                                        "Baby Medicine"};
  std::vector<int> itemPrices = {100, 50, 50,
                                 500};  // Prices for the baby items
  ...
  const float startX = (1920 - (columns * (itemWidth + horizontalSpacing))) /
                       2.0f;    // Center the grid horizontally
  ...
  for (size_t i = 0; i < itemNames.size(); ++i) {
    ...
    item.quantityText.setPosition(
        ...
        yPosition + item.sprite.getGlobalBounds().height +
            15  // Increased spacing below the image
    );
