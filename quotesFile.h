...
void updateQuote() {
    // Check if it's time to switch quotes
    if (quote_clock.getElapsedTime().asSeconds() >= newQuoteInterval) {
        currentQuote = getRandomQuote(petType);
        quote_clock.restart();                    
        quoteVisible = true;                    
    }

 
    if (quoteVisible && quote_clock.getElapsedTime().asSeconds() >= displayDuration) {
        quoteVisible = false;                 
    }
}
